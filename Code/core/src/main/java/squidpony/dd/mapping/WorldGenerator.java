...
import squidpony.dd.data.Material;
import squidpony.dd.data.Physical;
import squidpony.dd.data.Stone;
...
public class WorldGenerator {
    ...
    private Map<Material, Physical> walls = new HashMap<>();
...
//        }
            ...
            for (int y = 0; y < height; y++) {
                ...
                switch (c) {
                    case '.':
                        break;
                    case '#':
                        adding = getWall(tile.floor.sourceMaterial);
                        tile.add(adding);
                        break;
                    case '+':
                        Material stone = tile.floor.sourceMaterial;
                        Physical door = new Physical(stone);
                        door.glyph = '+';
                        tile.add(door);
                        break;
                    default:
                        tile.floor = new Physical(tile.floor); // Copy out the old floor before modifying it
                        tile.floor.glyph = eMap.altSymbolOf(c);
    ...
    private Physical getWall(Material stone){
        Physical wall = walls.get(stone);
        if (wall != null){
            return wall;
        }

        wall = new Physical(stone);
    ...
    private void mineralPlacement() {
        Physical floor = new Physical(rng.getRandomElement(Stone.values()));
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
    ...
    private void makeSolid(){
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                for (int z = 0; z < depth; z++) {
                    EpiTile tile = world[z].contents[x][y];
                    Physical p = new Physical(tile.floor);
                    p.glyph = '#';
                    tile.add(p);
                }
            }
        ...
        for (int growStep = 0; growStep < quantity; growStep++) {
            if (counter <= 0) {
                ...
                if (useExistingFloor) {
                    ...
                    blueprint = new Physical(rng.getRandomElement(Stone.values()));
                }
            }
            ...
            for (int x = currentX - forceX; x < currentX + forceX; x++) {
                for (int y = currentY - forceY; y < currentY + forceY; y++) {
                    if (pointInBounds(x, y, z)) {
                        world[z].contents[x][y].floor = new Physical(intruder); // TODO - get from cache
                    }
    ...
    private void extrudeMap() {
        ...
        for (int testZ = 0; testZ < depth; testZ++) {
            for (int n = 0; n < maxRecurse; n++) {
                ...
                if (blueprint.sourceMaterial instanceof Stone) {
                    Stone check = (Stone) blueprint.sourceMaterial;
                    if (check.extrusive || check.intrusive) {
                        extrudeZ = testZ;
                ...
                for (int x = extrudeX - sizeX; x < extrudeX + sizeX; x++) {
                    for (int y = extrudeY - sizeY; y < extrudeY + sizeY; y++) {
                        ...
                        if (n < 1) {//code for oval shape
                            if (pointInBounds(x, y, z)) {
                                world[z].contents[x][y].floor = new Physical(extruder); // TODO - cache
                            }
                        }
                    }
                }
                    ...
                    for (Physical testing1[][] : near) {
                        for (Physical testing2[] : testing1) {
                            for (Physical test : testing2) {
                                if (test.sourceMaterial instanceof Stone) {
                                    Stone check = (Stone) test.sourceMaterial;
                                    if (check.sedimentary) {
                                        sedimentary = true;
                                    }
                                    if (check.extrusive || check.intrusive) {
                                        igneous = true;
                    ...
                    if (changing) {
                        if (pointInBounds(i, k, j)) {
                            if (rng.nextInt(100) < 45) {
                                world[j].contents[i][k].floor = new Physical(changer); // TODO - cache
                            }
                        }
                    }
                }
