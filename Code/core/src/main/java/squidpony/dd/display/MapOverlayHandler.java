...
public class MapOverlayHandler {
    ...
    private void showEquipment(Direction moveSelection){
        ...
        put(xOffset, y, "Weapon", headingColor);
        y++;

        Physical p = player.weapon;
        int x = xOffset + 1;
        put(x, y, p.glyph, p.color);
        Coord select = Coord.get(x - 1, y);
        ...
        put(xOffset, y, "Armor", headingColor);
        y++;
        p = player.armor;
        x = xOffset + 1;
        put(x, y, p.glyph, p.color);
        select = Coord.get(x - 1, y);
        rightSelectables.add(select);
        selectables.put(select, p);
        x += 2;
        putWithRarityColor(x, y, p);
        ...
        y++;
        put(xOffset, y, "Jewelry", headingColor);
        y++;
        p = player.jewelry;
        x = xOffset + 1;
        put(x, y, p.glyph, p.color);
    ...
    private int getDisplayQuantity(Physical p) {
        return Integer.min(0, p.quantity);
    }
